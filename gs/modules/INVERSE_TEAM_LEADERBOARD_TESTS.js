...
function joinTeams(players){
    ...
    for(i in players){
        ...
        // player 4 and 5 join the second team
            ...
            Spark.sendRequestAs(joinTeamRequest, players[i].userId);
        // finally player 6 joins the third team
        } else if(i == 5){
...
suite("Inverse Team Leaderboards basic tests", function(){
    
    var leaderboard = Spark.getLeaderboards().getInverseTeamLeaderboard("CCL", "10001","10003");
    ...
    test("getLeaderboard, returns null when a Leaderboard doesn't exist", function(){
        assertThat(Spark.getLeaderboards().getInverseTeamLeaderboard("invalid_shortcode", "10001","10002","10003"), is(null));
    });
    
    test("getLeaderboard, returns an empty leaderboard, when teamIds are not valid", function(){
        assertThat(Spark.getLeaderboards().getInverseTeamLeaderboard(leaderboardShortcode, "invalid_teamId").getEntryCount(), is(0));
    });
    
    test("getLeaderboard with null leaderboard, Throws Null Pointer Exception", function(){
        clearLeaderboard(leaderboardShortcode);
        try{
        Spark.getLeaderboards().getInverseTeamLeaderboard(null, "10001","10002","10003");
        fail();
         } catch(e){
    ...
    test("getLeaderboard with null teamIds", function(){
        clearLeaderboard(leaderboardShortcode);
        assertThat(Spark.getLeaderboards().getInverseTeamLeaderboard(leaderboardShortcode, null).getEntryCount(), is(0));
    });
    ...
    test("getEntries, return false when no leaderboard Entries are present", function(){
        clearLeaderboard(leaderboardShortcode);
        assertThat(leaderboard.getEntries().hasNext(), is(false));
    });
    
    test("getEntries, return true when leaderboard Entries are present", function(){
        setTeamLeaderboardData(leaderboardShortcode);
        assertThat(leaderboard.getEntries().hasNext(), is(true));
    });
    
    test("getEntries, return false when no leaderboard Entries are present, using count & offset", function(){
        clearLeaderboard(leaderboardShortcode);
        assertThat(leaderboard.getEntries(10, 0).hasNext(), is(false));
    });
    
    test("getEntries, return true when leaderboard Entries are present, using count & offset", function(){
        setTeamLeaderboardData(leaderboardShortcode);
    ...
    test("getEntriesFromPlayer, returns the cursor over the leaderboard entries from the supplied playerId", function(){
        var playerId = setTeamLeaderboardData(leaderboardShortcode).reverse()[2].userId;
        
        var cursor = leaderboard.getEntriesFromPlayer(playerId, 10);
        var first = cursor.next();
        assertThat(first.getAttribute("SCORE"), is(400));
        assertThat(first.getRank(), is(2));
        assertThat(first.getRankPercentage(), is(66));
        assertThat(first.getUserId(), is(playerId));
        assertThat(first.getUserName(), is("cloudDisplayName_4"));
...
suite("Inverse Team Leaderboards Entry tests", function(){
...
suite("Inverse Team Leaderboards union/intersection/difference tests", function(){
    
    var leaderboardShortcode = "CCL";
    var players = setTeamLeaderboardData(leaderboardShortcode);
    var leaderboard1 = Spark.getLeaderboards().getInverseTeamLeaderboard("CCL", "10001");
    var leaderboard2 = Spark.getLeaderboards().getInverseTeamLeaderboard("CCL", "10002");
    var leaderboard3 = Spark.getLeaderboards().getInverseTeamLeaderboard("CCL", "10003");
    ...
    // needs reviewing
    test("test difference between 2 leaderboard with 2 players in common", function(){
        var difference = Spark.getLeaderboards().difference(leaderboard1, leaderboard2).evaluate();
